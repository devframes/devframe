...
// Hosted base (`/__devframes_plugin_og/`): the SPA and the dev bridge share it
// so `document.baseURI` matches production while Vite serves the HMR panel.
const basePath = resolveBasePath(devframe, 'hosted')

/**
 * Serve-only plugin that bridges the og node side (RPC + WebSocket +
 ...
 * `/__devframes_plugin_og/` (mounted in a hub). `connectDevframe` resolves
 ...
 * `document.baseURI` matches production, and {@link ogDevBridge} bridges the
...
export default defineConfig(({ command }) => ({
  ...
  resolve: { alias },
  plugins: [vue(), UnoCSS(), ogDevBridge()],
