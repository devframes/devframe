#!/usr/bin/env node
/**
 * Standalone demo host for the terminals panel.
 *
 * `initDevframe` runs setup, serves the built Svelte panel SPA + `__connection.json`
...
import createTerminalsDevframe from '../src/node/index.ts'

const HERE = fileURLToPath(new URL('.', import.meta.url))
const ROOT = resolve(HERE, '..')
const panelDir = resolve(ROOT, 'assets-pkg/dist')

const devframe = createTerminalsDevframe({ auth: false })
const basePath = devframe.basePath

async function main() {
  if (!existsSync(resolve(panelDir, 'index.html'))) {
    console.error('\n[terminals playground] missing assets-pkg/dist/index.html\n  → run `pnpm -C plugins/terminals build` first.\n')
    process.exit(1)
  }

  const bindHost = '0.0.0.0'
  const port = await getPort({ host: bindHost, port: 4411 })
  ...
  process.stdout.write(`\n  Terminals: dev (live WebSocket RPC)\n  ▸ panel:  ${origin}${basePath}\n\n`)
}
