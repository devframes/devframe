...
describe('dev-server (CLI surface)', () => {
  ...
  it('answers get-config over WebSocket RPC', async () => {
    ...
    const config = await rpc.$call('devframes:plugin:a11y:get-config') as {
      ...
      pageScript: { autoScan: boolean, logIssues: boolean, activateDockId: string }
      impacts: { id: string }[]
    }
    ...
    // Runtime config the panel forwards to the page script.
    expect(config.dockId).toBe('devframes_plugin_a11y')
    expect(config.defaultHighlight).toBe(false)
    expect(config.pageScript).toMatchObject({ autoScan: true, logIssues: true, activateDockId: 'devframes_plugin_a11y' })
  })
})
