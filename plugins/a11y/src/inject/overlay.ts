  ...
  return {
    ...
    preview(el, info) {
      if (isRootElement(el)) {
        ...
        showNotice(IMPACT_COLOR[info.impact], `${info.impact} · ${info.ruleId} on the whole page`)
        return
      }
      preview.el = el
    ...
    setPins(next) {
      ...
      if (rootPin)
        showNotice(IMPACT_COLOR[(rootPin as PinInfo).impact], `${(rootPin as PinInfo).number} · ${(rootPin as PinInfo).ruleId} on the whole page`)
      else if (!preview.el)
        hideNotice()
