...
shared state, which is replayed to late-joining panels and streamed as patches on every
...
Open the URL, then hover any row in the panel, and the matching element in the page
...
| `src/rpc/` | internal | `get-config` static RPC + the type-safe client registry |
| `src/shared/protocol.ts` | internal | the page script ↔ panel in-page channel contract (`A11yChannelProtocol`) |
| `src/inject/` | internal | the page script (axe scan, highlight overlay, hub messages mirror) → `dist/inject/inject.js` |
| `src/spa/` | internal | the Solid panel SPA → `assets-pkg/dist` (ships in `@devframes/plugin-a11y--assets`) |
| `demo/` | internal | same-origin host page + server (dev + static modes) |
| `tests/` | internal | dev-server RPC + static-build dump |
