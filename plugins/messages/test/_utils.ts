...
function localDistDir(): string {
  const resolved = resolveStaticAssetsSource(messagesDevframe.clientAssets!, path.join(os.tmpdir(), 'devframes_plugin_messages-test'), messagesDevframe.importMetaUrl)
