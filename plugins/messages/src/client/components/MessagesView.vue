...
import { fromEntries, getHashColorFromString, levelPriority, levels } from './MessageItemConstants'
...
const allLevels: DevframeMessageLevel[] = Object.keys(levels) as DevframeMessageLevel[]
const allSources: DevframeMessageEntryFrom[] = Object.keys(fromEntries) as DevframeMessageEntryFrom[]
...
const activeFilters = ref<Set<DevframeMessageLevel>>(new Set())
const activeLabelFilters = ref<Set<string>>(new Set())
const activeFromFilters = ref<Set<DevframeMessageEntryFrom>>(new Set())
...
function toggleFilter(level: string) {
  ...
  if (filters.has(level as DevframeMessageLevel))
    filters.delete(level as DevframeMessageLevel)
  else
    filters.add(level as DevframeMessageLevel)
}
...
function toggleFrom(from: DevframeMessageEntryFrom) {
...
const filteredEntries = computed(() => {
  ...
  if (activeFromFilters.value.size > 0)
    entries = entries.filter(e => activeFromFilters.value.has(e.from as DevframeMessageEntryFrom))
...
function openFile(entry: DevframeMessageEntry) {
...
function dismissFiltered() {
  emit('dismissMany', filteredEntries.value.map(e => e.id))
  selectedId.value = null
}
</script>
    ...
    <div class="border-base border-b p3 flex flex-col gap-2">
      ...
      <div class="flex items-center gap-1">
        <input
          ...
          class="bg-transparent border border-base rounded px-2 py-0.5 text-xs w-48 outline-none focus:border-active"
        >
        <button
        ...
        <div class="flex-1" />
        <span v-if="filteredEntries.length !== entries.length" class="text-xs op40">
      ...
      <div class="flex flex-wrap items-center gap-1">
        ...
        <FilterToggles
          label="From"
          :items="allSources"
          :active="(activeFromFilters as Set<DevframeMessageEntryFrom>)"
          :styles="fromEntries"
    ...
    <div class="h-full of-hidden" :class="selectedEntry ? 'grid grid-cols-[1fr_1fr]' : ''">
      ...
      <div v-if="selectedEntry" class="h-full of-y-auto border-l border-base p-4">
        ...
        <div class="flex flex-wrap items-center gap-2 mb-3 text-xs">
          ...
          <span v-if="fromEntries[selectedEntry.from as DevframeMessageEntryFrom]" class="flex items-center gap-1" :class="fromEntries[selectedEntry.from as DevframeMessageEntryFrom].color">
            <div :class="fromEntries[selectedEntry.from as DevframeMessageEntryFrom].icon" class="w-3.5 h-3.5" />
            {{ fromEntries[selectedEntry.from as DevframeMessageEntryFrom].label }}
          </span>
        ...
        <div
          v-else-if="selectedEntry.filePosition"
