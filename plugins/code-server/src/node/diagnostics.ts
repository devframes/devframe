...
export const diagnostics = defineDiagnostics({
  ...
  codes: {
    DP_CODE_SERVER_0001: {
      why: (p: { bin: string }) =>
        `code-server is not installed (could not run "${p.bin} --version")`,
      fix: 'Install code-server — e.g. `curl -fsSL https://code-server.dev/install.sh | sh` — or set the `bin` option to its path. See https://coder.com/docs/code-server/latest/install',
    },
    DP_CODE_SERVER_0002: {
      why: (p: { port: number, timeout: number }) =>
        `code-server did not become ready on port ${p.port} within ${p.timeout}ms`,
      fix: 'Check the code-server logs for startup errors, raise `startTimeout`, or free the port.',
    },
    DP_CODE_SERVER_0003: {
      why: (p: { bin: string, reason: string }) =>
        `Failed to spawn code-server ("${p.bin}"): ${p.reason}`,
    },
    DP_CODE_SERVER_0004: {
    ...
    DP_CODE_SERVER_0005: {
      why: (p: { code: number }) =>
        `code-server exited unexpectedly with code ${p.code}`,
