#!/usr/bin/env node
/**
 * Standalone demo host for the code-server launcher.
 *
...
import createCodeServerDevframe from '../src/node/index.ts'
...
async function main() {
  if (!existsSync(resolve(panelDir, 'index.html'))) {
    console.error('\n[code-server playground] missing assets-pkg/dist/index.html\n  → run `pnpm -C plugins/code-server build` first.\n')
    process.exit(1)
  }

  const bindHost = '0.0.0.0'
  const port = await getPort({ host: bindHost, port: 4413 })
  ...
  process.stdout.write(`\n  Code Server launcher: dev (live WebSocket RPC)\n  ▸ panel:  ${origin}${basePath}\n\n`)
}
