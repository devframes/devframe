...
const devframe = createCodeServerDevframe()
// Hosted base (`/__devframes_plugin_code-server/`): the SPA and the dev bridge
// share it so `document.baseURI` matches production while Vite serves the HMR
// panel.
const basePath = resolveBasePath(devframe, 'hosted')

/**
 * Serve-only plugin that bridges the code-server node side (RPC + WebSocket +
 ...
 */
function codeServerDevBridge(): Plugin {
  return {
    name: 'code-server-dev-bridge',
    apply: 'serve',
 ...
 * `/__devframes_plugin_code-server/` (mounted in a hub). `connectDevframe`
 ...
 * `document.baseURI` matches production, and {@link codeServerDevBridge}
...
export default defineConfig(({ command }) => ({
  ...
  resolve: { alias },
  plugins: [vue(), UnoCSS(), codeServerDevBridge()],
