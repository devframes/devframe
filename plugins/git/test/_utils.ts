...
/**
 * Build a node context rooted at `cwd`. Used directly by the build-dump
 * test and indirectly by {@link startDashboardServer}.
 */
 ...
 */
export async function startDashboardServer(cwd: string): Promise<DashboardServer> {
  ...
  const app = new H3()
  const ctx = await createDashboardContext(cwd, 'dev', app)
