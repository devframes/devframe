...
function PatchViewer({ staged, path }: { staged: boolean, path: string }) {
  const loader = useCallback(
    (rpc: DevframeRpcClient) => rpc.call('devframes:service:git:diff', { staged, path }),
    [staged, path],
  )
...
export function StatusPanel() {
  const { rpc } = useRpc()
  const loader = useCallback((r: DevframeRpcClient) => r.call('devframes:service:git:status'), [])
  ...
  const stage = useCallback(async (paths: string[]) => {
    ...
    setNote(null)
    try {
      setData(await rpc.call('devframes:service:git:stage', { paths }))
    }
    finally {
      setBusy(false)
    }
  ...
  const unstage = useCallback(async (paths: string[]) => {
    ...
    setNote(null)
    try {
      setData(await rpc.call('devframes:service:git:unstage', { paths }))
    }
    finally {
      setBusy(false)
    }
  ...
  const commit = useCallback(async () => {
    ...
    setNote(null)
    try {
      const result = await rpc.call('devframes:service:git:commit', { message })
