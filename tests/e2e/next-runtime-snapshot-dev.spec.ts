...
test.describe('next-runtime-snapshot (dev)', () => {
  ...
  test('memory card populates and refresh re-invokes the RPC', async ({ page }) => {
    const memCard = page.locator('.df-card').filter({ hasText: 'Memory & Uptime' })
    ...
    await expect(memCard.locator('span.text-muted-foreground').first()).toHaveText('uptime')
  })

  test('env filter triggers a query call', async ({ page }) => {
    const envCard = page.locator('.df-card').filter({ hasText: 'Environment' })
    // Default pattern "NODE" should yield at least one row on most systems.
    await expect(envCard.locator('div.scrollbar-slim')).toBeVisible({ timeout: 10_000 })

    const input = envCard.locator('input')
    await input.fill('___definitely_not_a_real_env_var___')
    await expect(envCard.locator('p.text-muted-foreground')).toBeVisible({ timeout: 5_000 })
    await expect(envCard.locator('p.text-muted-foreground')).toContainText('No environment variables match')
  })
