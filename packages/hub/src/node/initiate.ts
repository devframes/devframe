...
import { applyClientModuleResolutionTemplate, isBareModuleSpecifier } from '../client-modules'
...
export interface InitHubOptions {
   ...
   * applied by every client-script loader (`createDevframeClientHost`, the
   * hub-ui viewers, `__client-imports.js`).
   *
   * Declare it only when the host can actually serve npm modules to the
   * browser — a Vite host passes `'/@id/{specifier}'` (requests route through
   * Vite's own resolution and import-analysis, sharing the app's module
   * graph; `@devframes/vite/hub` declares this by default). Left undeclared,
   * bare specifiers are unsupported on this host and registering one warns
   * `DF8111` — ship a self-contained bundle and pass its URL instead.
   */
...
function renderClientImportsModule(ctx: DevframeHubContext): string {
  ...
  const specifierOf = (script: ClientScriptEntry): string =>
    template && isBareModuleSpecifier(script.importFrom)
      ? applyClientModuleResolutionTemplate(template, script.importFrom)
      : script.importFrom
  ...
  for (const [id, view] of ctx.docks.views) {
    ...
    if (scripts.length === 0)
      continue
    const thunks = scripts.map(script => `() => import(${JSON.stringify(specifierOf(script))})`)
