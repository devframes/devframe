...
export const diagnostics = defineDiagnostics({
  ...
  codes: {
    ...
    DF8111: {
      ...
      fix: 'Run under a host framework that declares `initHub({ clientModuleResolution })` (e.g. Vite\'s `\'/@id/{specifier}\'`), ship the script as a self-contained bundle served by URL, or resolve it in the hub UI provider via `createDevframeClientHost({ resolveClientModule })` (then disregard this warning).',
    },
    DF8200: {
