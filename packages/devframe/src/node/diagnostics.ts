...
export const diagnostics = defineDiagnostics({
  ...
  codes: {
    ...
    DF0042: {
      why: (p: { id: string }) => `"${p.id}" declares \`capabilities.build: false\`, so its static export is not meaningful (writes are excluded and any live-served data won't be there).`,
    ...
    DF0058: {
      why: (p: { id: string }) => `"${p.id}" declares \`capabilities.dev: false\`, so it does not support a live dev server (its value is a static export only).`,
    ...
    DF0072: {
      why: (p: { method: string }) =>
        `\`rpc.snapshot\` names "${p.method}", but no RPC function is registered under that id, so nothing to bake into the static build.`,
