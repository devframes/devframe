...
export async function checkClientDist(options: CheckClientDistOptions): Promise<void> {
  ...
  async function visit(file: string): Promise<void> {
    ...
    // Static imports load eagerly when the file is evaluated, so they're the
    // leak vector this guard exists to catch. Flag any forbidden specifier.
    ...
    // checked against FORBIDDEN, but the chunk they target is, on visit.
