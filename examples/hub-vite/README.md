...
A tiny, copyable **vite-devtools-style hub**. [vite-devtools](https://github.com/vitejs/devtools) is the full viewer that docks many Vite tools behind one icon rail on top of `@devframes/hub`; this example is the smallest thing shaped like it - an icon dock, an iframe stage, and a drawer of hub subsystems - so you can see the whole protocol and build your own viewer from it.
...
The dock UI is plain DOM in `src/client/`. To skin your own viewer, read the same shared-state keys and render them however you like; swap the inline `icons.ts` for your framework's icon component (UnoCSS `preset-icons`, `@iconify/vue`, …). The node-side file is the part worth copying verbatim.
