...
the action bridge reports actions as unavailable, because there is no live RPC.
...
it onto a `json-render` dock: the [Vite hub](../vite-devframe-hub)
renders it with `@devframes/json-render-ui` (Vue), and the
[Next hub](../next-devframe-hub) renders it with a small React registry.
