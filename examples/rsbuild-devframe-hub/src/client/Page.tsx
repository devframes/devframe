...
// The mini React json-render registry is shared verbatim with the Next hub
// example — a React host renders a server-authored spec with its own components
// (the "registry replacement" path) instead of the Vue reference frontend.
import { createReactJsonRenderDockRenderer } from '../../../next-devframe-hub/src/client/json-render/dock-renderer'
      ...
      try {
        ...
        const refreshMessages = async () => {
          const entries = await rpc.call(
            'example:rsbuild-devframe-hub:messages:list' as any,
        ...
        const refreshTerminals = async () => {
          const sessions = await rpc.call(
            'example:rsbuild-devframe-hub:terminals:list' as any,
          ) as TerminalSummary[]
          if (!cancelled)
  ...
  async function ping() {
    ...
    try {
      const result = await rpcRef.current.call(
        ...
        'example:rsbuild-devframe-hub:ping',
      )
    ...
    <div className="h-full flex flex-col bg-base color-base">
      <header className="shrink-0 flex items-center gap-3 h-nav px-3 border-b border-base bg-base">
        <h1 className="m0 flex items-center gap-1.5 shrink-0 text-sm font-semibold select-none">
          ...
          <span>Rsbuild Devframe Hub</span>
        </h1>
