...
export default defineDevframe({
  id: 'example:rsbuild-demo-tool',
  name: 'Rsbuild Demo Tool',
  version: pkg.version,
  packageName: pkg.name,
  homepage: pkg.homepage,
  description: 'A tiny demo devframe that plugs into the hub via mountDevframe().',
  icon: 'ph:rocket-duotone',
  basePath: '/__rsbuild-demo-tool/',
  cli: {
    distDir: fileURLToPath(new URL('../spa/rsbuild-demo-tool/', import.meta.url)),
  },
  async setup(rawCtx) {
    const ctx = rawCtx as unknown as DevframeHubContext

    ctx.commands.register({
      id: 'example:rsbuild-demo-tool:say-hello',
