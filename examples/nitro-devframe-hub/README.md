...
- The RPC WebSocket runs on an eager side-car port (Nitro handlers hand over `Request`s, so the socket lives on its own port), advertised through `__connection.json`; the browser client discovers it automatically.
