...
- [`src/hub.ts`](./src/hub.ts): `initHub({ devframes, ui: createUi({ branding }) })` (rebranded to Svelte's own orange, `#ff3e00`), memoized on `globalThis` so SvelteKit's dev-time reload reuses the live hub. The RPC socket runs on a side-car port (`ws: { sidecar: true }`) advertised via `__connection.json`, because SvelteKit's `+server.ts` handlers hand over `Request`s and never see WebSocket upgrades, so the hub takes a socket of its own; the browser client discovers it automatically.
