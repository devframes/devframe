...
- The public `devframe/node` barrel is reduced to the context surface only:
  `createHostContext` (+ options), `createStorage` (+ options), and the
  `RpcFunctionsHost` type. Serving is done through the adapters or
  `devframe/initiate`.
- A new **`devframe/internal`** entry (an explicitly-unstable cross-package
  ...
  `createH3DevframeHost`, `startHttpAndWs` (+ `StartedServer`),
  ...
  + types), and `normalizeHttpServerUrl`. The hub, the inspect plugin, the host
  examples, and every plugin's test harness import from there.
- `isObject` is **removed entirely** (unused internally; trivial to inline).
- The remaining node internals (diagnostics/services/views hosts, the
  streaming/shared-state/scope/settings factories, `toDialableHost`/
  `formatHostForUrl`) stay fully internal (relative imports only).
