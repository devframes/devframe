---
title: 'DF0037: Duplicate Service Provider'
description: 'A service is already provided under "{id}".'
---
...
`ctx.services` holds exactly one provider per service id. A second `provide()` under the same id throws instead of silently replacing a service another devframe may already hold a reference to.
...
- If the collision is between two unrelated devframes, namespace the id with your devframe id (`<plugin-id>:<service>`), the same rule RPC function names follow.
