...
A `rpc.snapshot` entry references a method id the context has no RPC function for, so the static collector has nothing to dump. Typically the id is misspelled, or the service/devframe that registers it isn't installed when the build runs.

## Fix

Check the method id, and ensure the service/devframe that registers it is installed (e.g. declared in `services`) before the build collects the dump.
