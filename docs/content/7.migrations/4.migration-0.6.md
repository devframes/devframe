...
RPC client calls (`requestTrustWithToken()`, `requestTrustWithCode()`) are unchanged; update only node-side handlers or `authorize` gates using the old names.

## WebSocket connections now enforce origin and (optionally) trust

Two gates landed on the RPC socket:

- **Cross-origin upgrades rejected by default.** Only loopback origins (`localhost`/`127.0.0.1`/`::1`) and `Origin`-less requests are accepted; another origin needs an explicit allowlist:
