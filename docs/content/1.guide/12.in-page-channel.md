...
Functions are defined with `defineChannelFunction` — the same authoring shape as `defineRpcFunction` (`name`, `type`, Standard-Schema `args`/`returns`, `jsonSerializable`, `handler`/`setup`), narrowed to the browser. Define each side's functions in that side's source files; the shared protocol file carries only types.
...
const pageScript = createPageScriptChannel<MyChannelProtocol>({ name: MY_CHANNEL, transport: port1 })
