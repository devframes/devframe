...
`base` is required: pass `resolveBasePath(def, 'hosted')` (`def.basePath ?? /__<id>/`) to default it, and the running devframe echoes it back as `devtools.base`. `handler`/`nodeMiddleware` await readiness internally. The running devframe binds no port, so [the WebSocket binding](#the-websocket-binding) is the host framework's call.
