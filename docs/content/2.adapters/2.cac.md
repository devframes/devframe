...
```sh
my-tool                         # dev server at http://localhost:9999/
my-tool --port 8080
...
The SPA serves at `/` standalone, `/__my-tool/` hosted ([Mount paths](/adapters/#mount-paths)).
...
| `onReady` | none | `(info: { origin, port, app }) => void \| Promise<void>`, called once listening. |
