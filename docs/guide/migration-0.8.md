...
0.8 makes RPC schemas validator-neutral and runtime-validated, upgrades the MCP adapter to `@modelcontextprotocol/sdk` v2, and adds the agent-native MCP surface. See the [v0.8.0 release notes](https://github.com/devframes/devframe/releases/tag/v0.8.0).
...
0.8 adds the agent host on `ctx.agent` — `registerTool`, `registerToolProvider`, `registerResource` — plus an instance registry and the `devframe connect` MCP connector. Existing `agent`-flagged RPCs keep working.
...
See [Agent-Native](./agent-native) and [MCP → `devframe connect`](/adapters/mcp#discovery-devframe-connect).
